...
export default class extends TwoCanvasAnswer {
  ...
  main(canvas1: HTMLCanvasElement, image: HTMLImageElement) {
    ...
    const norm = (arr: number[], callback: ((value: number) => number) | null = null) => {
    ...
    const [gray, Ix, Iy, Ix2, Iy2, IxIy, _hes, Ix2t, Iy2t, Ixyt] = Array.from(new Array(10), () =>
    ...
    obj.forEach((e) => {
      ...
      this.showMessage(`<strong>${e.label}</strong><br><img src="${url}"/>`, true)
    })
  }
  /**
   * ガウシアンカーネル生成
