...
export default class extends ThreeCanvasAnswer {
  ...
  main(canvas1: HTMLCanvasElement, canvas2: HTMLCanvasElement, image: HTMLImageElement) {
    ...
    template.onload = () => {
      ...
      const [x, y] = this.templateMatchingNCC(
        src2.data,
        image.width,
        image.height,
        src1.data,
   ...
   * @return {Array} pos
   */
  templateMatchingNCC(src: Pixels, W: number, H: number, template: Pixels, tW: number, tH: number) {
    ...
    for (let y = 0; y < H - tH; y++)
      for (let x = 0; x < W - tW; x++) {
        ...
        const sum = (A / Math.sqrt(B)) * Math.sqrt(C)
        if (sum > v) {
          v = sum
          resultX = x
          resultY = y
