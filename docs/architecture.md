...
UI を作り替えても 100 問の実装に手を入れる必要がない。実際、Vue 2 から React への移行では
画像処理のロジックを一行も変えていない。
...
| math     | （未使用）                          | 削除                                            |
| OpenCV   | 画像処理・特徴抽出・物体検出        | `src/questions/answers/` と `src/lib/vision.ts` |
...
404.html を index.html の複製にすると HTTP ステータスが 404 のままになり検索エンジンに拾われない。
そこで次の 2 段構えにしている（`vite.config.ts` の `githubPagesSpaFallback`）。

1. `404.html` がパスをクエリ文字列に畳んで `index.html` へリダイレクトする
2. `index.html` 冒頭のインラインスクリプトが `history.replaceState` で元のパスへ戻す

結果として、最終的なレスポンスは 200 になり、アドレスバーにはクリーン URL が残る。
...
- `src/lib/vision.test.ts` — IoU・NMS・HOG・k-means の基本性質
